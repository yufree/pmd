...
When retention time is not provided, m/z vector can still be used to check reaction level changes. You might use `getrda` to find the high frequncy PMDs. Then `getpmddf` function can be used to extract all the paired ions for certain PMD.
...
# show the PMD dynamic intensity
pmdinsdynamicall
```
