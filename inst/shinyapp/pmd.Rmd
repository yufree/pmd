---
title: "Paired Mass Differences(PMD) analysis"
author: "Miao Yu"
date: "`r Sys.Date()`"
...
inputPanel(
  ...
  sliderInput("freqcutoff", label = "cutoff of freqency of PMDs between RT cluster for independent peaks:",
...
datastd <- reactive({
        list <- datainput()
        if(!is.null(list)){
                list <- globalstd(list,input$rtcutoff,input$ng,freqcutoff = input$freqcutoff)
        }
        return(list)
})
stdcsv <- reactive({
